    ...
    waitForTx(result.txHash)
      .then(async (status) => {
        if (status.success) {
          ...
          const PROPOSED_SIG = ethers.id(
            "RegistryInitProposed(address,string,uint256)",
          );
          for (const log of status.receipt?.logs || []) {
            if (
              log.topics?.[0]?.toLowerCase() === PROPOSED_SIG.toLowerCase() &&
              log.topics[1]
            ) {
              try {
                cloneAddress = ethers.getAddress(
          ...
          if (!cloneAddress) {
            console.error(
              "❌ Could not parse clone address from RegistryInitProposed event",
            );
            return;
          }
          ...
          console.log(
            `✅ Registry proposed: ${nspace} → clone=${cloneAddress}`,
          );
        } else {
      ...
      .then(async (status) => {
        if (status.success) {
          ...
          if (p?.isWallet) {
            ...
            console.log(
              `✅ Registry executed + added to WalletRegistry: ${cleanRegistry}`,
            );
          } else {
...
router.post("/propose-upgrade", requireValidator, async (req, res) => {
  try {
    ...
    if (!isMultisig && !cleanProxy)
      return res
        .status(400)
        .json({ message: "proxyAddress required when isMultisig=false" });
    ...
    if (existing)
      return res
        .status(409)
        .json({ message: "Upgrade proposal already exists for this impl" });

    console.log(
      `📋 proposeUpgrade: newImpl=${cleanNewImpl} isMultisig=${isMultisig}`,
    );
...
router.post("/propose-signer-update", requireValidator, async (req, res) => {
  try {
    ...
    if (!cleanProxy || !cleanSigner)
      return res
        .status(400)
        .json({ message: "factoryProxy and newSigner required" });
    ...
    if (existing)
      return res
        .status(409)
        .json({ message: "Signer update proposal already exists" });

    console.log(
      `📋 proposeSignerUpdate: ${cleanSigner} via factory=${cleanProxy}`,
    );
    ...
    waitForTx(result.txHash)
      .then(async (status) => {
        if (status.success) {
          ...
          console.log(
            `✅ Signer update proposal synced — remaining=${remaining}`,
          );
        } else {
          await Proposal.deleteOne({ _id: proposal._id });
          console.error(
            "❌ propose-signer-update tx failed — proposal removed",
          );
        }
      })
      .catch((e) =>
        console.error("❌ propose-signer-update bg error:", e.message),
      );
  } catch (error) {
...
router.post("/validate-signer-update", requireValidator, async (req, res) => {
  try {
    ...
    const cleanSigner = normalizeAddr(newSigner);
    if (!cleanSigner)
      return res.status(400).json({ message: "Invalid newSigner address" });
...
router.post("/execute-signer-update", requireValidator, async (req, res) => {
  try {
    ...
    waitForTx(result.txHash)
      ...
      .catch((e) =>
        console.error("❌ execute-signer-update bg error:", e.message),
      );
  } catch (error) {
...
router.post("/cancel-signer-update", requireValidator, async (req, res) => {
  try {
    ...
    waitForTx(result.txHash)
      ...
      .catch((e) =>
        console.error("❌ cancel-signer-update bg error:", e.message),
      );
  } catch (error) {
...
router.post("/update-factory-fee", requireValidator, async (req, res) => {
  try {
    ...
    const cleanProxy = normalizeAddr(factoryProxy);
    if (!cleanProxy)
      return res.status(400).json({ message: "Invalid factoryProxy address" });
...
router.post("/pause-state", requireValidator, async (req, res) => {
  try {
    ...
    const cleanProxy = normalizeAddr(proxyAddress);
    if (!cleanProxy)
      return res.status(400).json({ message: "Invalid proxyAddress" });
    if (mark === undefined)
      return res
        .status(400)
        .json({ message: "mark required (0=multisig, 1=external)" });
...
router.post("/propose-unpause", requireValidator, async (req, res) => {
  try {
    ...
    if (existing)
      return res
        .status(409)
        .json({ message: "Unpause proposal already exists for this proxy" });
...
router.post("/withdraw", requireValidator, async (req, res) => {
  try {
    const { privateKey, singletonAddress, tokenAddress, receiverAddress } =
      req.body;
    ...
    if (!cleanSingleton || !cleanToken || !cleanReceiver)
      return res
        ...
        .json({
          message:
            "singletonAddress, tokenAddress, and receiverAddress required",
        });

    console.log(
      `📋 withdrawFromSingleton: token=${cleanToken} receiver=${cleanReceiver}`,
    );
...
router.post("/update-recovery", requireValidator, async (req, res) => {
  try {
    ...
    const cleanTarget = normalizeAddr(targetAddress);
    if (!cleanTarget)
      return res.status(400).json({ message: "Invalid targetAddress" });
...
router.post("/emergency-add-validator", requireValidator, async (req, res) => {
  try {
    ...
    console.log(
      `🚨 Emergency: adding validator ${cleanTarget} via backend wallet directly`,
    );
