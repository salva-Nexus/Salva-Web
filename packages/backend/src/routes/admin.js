...
function normalizeAddr(addr) {
  if (!addr) return null;
  try {
    return ethers.getAddress((cleanEnvAddr(addr) || addr).toLowerCase());
  } catch {
    return null;
  }
}
...
router.get('/proposals', async (req, res) => {
  try {
    ...
    for (const p of all) {
      try {
        let remaining = null;
        if (p.type === 'registry' && p.registry)
          remaining = await getRemaining('registryInitVotesRemaining', p.registry);
        else if (p.type === 'validator' && p.addr)
          remaining = await getRemaining('validatorUpdateVotesRemaining', p.addr);
        else if (p.type === 'upgrade' && p.newImpl)
          remaining = await getRemaining('upgradeVotesRemaining', p.newImpl);
        else if (p.type === 'signerUpdate' && p.newImpl)
          remaining = await getRemaining('signerUpdateVotesRemaining', p.newImpl);
        else if (p.type === 'ImplUpdate' && p.newImpl)
          remaining = await getRemaining('ImplUpdateVotesRemaining', p.newImpl);
        else if (p.type === 'unpause' && p.proxy)
          remaining = await getRemaining('unpauseVotesRemaining', p.proxy);
    ...
    res.json({
      ...
      implUpdateProposals: all.filter((p) => p.type === 'ImplUpdate'),
  ...
  try {
    ...
    if (!singleton || !factory)
      return res.status(500).json({
        message: 'SALVA_SINGLETON or REGISTRY_FACTORY env var not set',
      });

    console.log(`🔍 ENV CHECK: singleton=${singleton} factory=${factory}`);

    console.log(`📋 proposeInitRegistry: ${nspace} (isWallet=${isWallet})`);

    const result = await relay.sponsorProposeInitRegistry(
    ...
    waitForTx(result.txHash)
      .then(async (status) => {
        if (status.success) {
          ...
          if (!cloneAddress) {
            console.error('❌ Could not parse clone address from RegistryInitProposed event');
            return;
          }
          ...
          await notifyAllValidators('New Registry Initialization Proposal', {
            type: 'registry',
...
router.post('/execute-registry', requireValidator, async (req, res) => {
  try {
    ...
    const result = await relay.sponsorExecuteInitRegistry(
      req.callerUser.safeAddress,
      privateKey,
      cleanRegistry
    );
...
router.post('/cancel-registry', requireValidator, async (req, res) => {
  try {
    ...
    const result = await relay.sponsorCancelRegistryInit(
      req.callerUser.safeAddress,
      privateKey,
      cleanRegistry
    );
...
router.post('/propose-validator', requireValidator, async (req, res) => {
  try {
    ...
    const result = await relay.sponsorProposeValidatorUpdate(
      req.callerUser.safeAddress,
      privateKey,
      cleanTarget,
      action
...
router.post('/validate-validator', requireValidator, async (req, res) => {
  try {
    ...
    const result = await relay.sponsorValidateValidatorUpdate(
      req.callerUser.safeAddress,
      privateKey,
      cleanTarget
    );
    ...
    try {
      ...
      console.log(
        `✅ Validator validated — remaining=${remaining}${isValidated ? ' (timelock started)' : ''}`
      );
    } catch (e) {
      console.error('❌ validate-validator bg error:', e.message);
    }
  } catch (error) {
...
router.post('/execute-validator', requireValidator, async (req, res) => {
  try {
    ...
    const result = await relay.sponsorExecuteValidatorUpdate(
      req.callerUser.safeAddress,
      privateKey,
      cleanTarget
    );
    ...
    waitForTx(result.txHash)
      .then(async (status) => {
        if (status.success) {
          ...
          if (updated)
            console.log(`✅ Validator executed: ${updated.username} isValidator=${action}`);
...
router.post('/cancel-validator', requireValidator, async (req, res) => {
  try {
    ...
    const result = await relay.sponsorCancelValidatorUpdate(
      req.callerUser.safeAddress,
      privateKey,
      cleanTarget
    );
...
router.post('/propose-upgrade', requireValidator, async (req, res) => {
  try {
    ...
    if (!isMultisig && !cleanProxy)
      return res.status(400).json({ message: 'proxyAddress required when isMultisig=false' });
    ...
    if (existing)
      return res.status(409).json({ message: 'Upgrade proposal already exists for this impl' });

    console.log(`📋 proposeUpgrade: newImpl=${cleanNewImpl} isMultisig=${isMultisig}`);

    const result = await relay.sponsorProposeUpgrade(
      req.callerUser.safeAddress,
      privateKey,
      isMultisig ? ethers.ZeroAddress : cleanProxy,
      cleanNewImpl,
      isMultisig
    );
    if (!result?.txHash)
    ...
    const proposal = await Proposal.create({
      type: 'upgrade',
      newImpl: cleanNewImpl.toLowerCase(),
      proxy: cleanProxy?.toLowerCase() || null,
      isMultisig,
      remainingValidation: null,
      isValidated: false,
      timeLockTimestamp: null,
...
router.post('/validate-upgrade', requireValidator, async (req, res) => {
  try {
    ...
    const result = await relay.sponsorValidateUpgrade(
      req.callerUser.safeAddress,
      privateKey,
      cleanNewImpl
    );
    ...
    try {
      ...
      console.log(
        `✅ Upgrade validated — remaining=${remaining}${isValidated ? ' (timelock started)' : ''}`
      );
    } catch (e) {
      console.error('❌ validate-upgrade bg error:', e.message);
    }
  } catch (error) {
...
router.post('/execute-upgrade', requireValidator, async (req, res) => {
  try {
    ...
    const result = await relay.sponsorExecuteUpgrade(
      req.callerUser.safeAddress,
      privateKey,
      cleanNewImpl
    );
...
router.post('/cancel-upgrade', requireValidator, async (req, res) => {
  try {
    ...
    const result = await relay.sponsorCancelUpgrade(
      req.callerUser.safeAddress,
      privateKey,
      cleanNewImpl
    );
...
router.post('/propose-signer-update', requireValidator, async (req, res) => {
  try {
    ...
    if (!cleanProxy || !cleanSigner)
      return res.status(400).json({ message: 'factoryProxy and newSigner required' });

    const existing = await Proposal.findOne({
      type: 'signerUpdate',
      newImpl: cleanSigner.toLowerCase(),
    });
    ...
    const result = await relay.sponsorProposeSignerUpdate(
      req.callerUser.safeAddress,
      privateKey,
      cleanProxy,
      cleanSigner
...
router.post('/validate-signer-update', requireValidator, async (req, res) => {
  try {
    ...
    const result = await relay.sponsorValidateSignerUpdate(
      req.callerUser.safeAddress,
      privateKey,
      cleanSigner
    );
    ...
    try {
      ...
      console.log(
        `✅ Signer update validated — remaining=${remaining}${isValidated ? ' (timelock started)' : ''}`
      );
    } catch (e) {
      console.error('❌ validate-signer-update bg error:', e.message);
    }
  } catch (error) {
...
router.post('/execute-signer-update', requireValidator, async (req, res) => {
  try {
    ...
    const result = await relay.sponsorExecuteSignerUpdate(
      req.callerUser.safeAddress,
      privateKey,
      cleanSigner
    );
...
router.post('/cancel-signer-update', requireValidator, async (req, res) => {
  try {
    ...
    const result = await relay.sponsorCancelSignerUpdate(
      req.callerUser.safeAddress,
      privateKey,
      cleanSigner
    );
...
router.post('/propose-impl-update', requireValidator, async (req, res) => {
  try {
    const { privateKey, factoryProxy, newImplAddress } = req.body;
    const cleanProxy = normalizeAddr(factoryProxy);
    const cleanNewImpl = normalizeAddr(newImplAddress);
    if (!cleanProxy || !cleanNewImpl)
      return res.status(400).json({ message: 'factoryProxy and newImplAddress required' });
    ...
    console.log(`📋 proposeImplUpdate: ${cleanNewImpl} via factory=${cleanProxy}`);
    ...
    console.log(`⏳ proposeImplUpdate tx submitted: ${result.txHash}`);
    res.json({ success: true, txHash: result.txHash, proposal });
    ...
    waitForTx(result.txHash)
      .then(async (status) => {
        if (status.success) {
          await new Promise((r) => setTimeout(r, 3000));
          const remaining = await getRemaining('ImplUpdateVotesRemaining', cleanNewImpl);
          await Proposal.updateOne({ _id: proposal._id }, { remainingValidation: remaining });
          console.log(`✅ Impl update proposal synced — remaining=${remaining}`);
        } else {
          await Proposal.deleteOne({ _id: proposal._id });
          console.error('❌ propose-impl-update tx failed — proposal removed');
        }
      })
      .catch((e) => console.error('❌ propose-impl-update bg error:', e.message));
  } catch (error) {
...
router.post('/validate-impl-update', requireValidator, async (req, res) => {
  try {
    const { privateKey, newImplAddress } = req.body;
    const cleanNewImpl = normalizeAddr(newImplAddress);
    if (!cleanNewImpl) return res.status(400).json({ message: 'Invalid newImpl address' });
    ...
    console.log(`⏳ validateImplUpdate tx submitted: ${result.txHash}`);
    res.json({ success: true, txHash: result.txHash });
...
router.post('/execute-impl-update', requireValidator, async (req, res) => {
  try {
    const { privateKey, newImplAddress } = req.body;
    const cleanNewImpl = normalizeAddr(newImplAddress);
    if (!cleanNewImpl) return res.status(400).json({ message: 'Invalid newImpl address' });
    ...
    waitForTx(result.txHash)
      ...
      .catch((e) => console.error('❌ execute-impl-update bg error:', e.message));
  } catch (error) {
...
router.post('/cancel-impl-update', requireValidator, async (req, res) => {
  try {
    const { privateKey, newImplAddress } = req.body;
    const cleanNewImpl = normalizeAddr(newImplAddress);
    if (!cleanNewImpl) return res.status(400).json({ message: 'Invalid newImpl address' });
    ...
    waitForTx(result.txHash)
      .then(async (status) => {
        if (status.success) {
          ...
          console.log(`✅ impl update cancelled: ${cleanNewImpl}`);
        }
      })
      .catch((e) => console.error('❌ cancel-impl-update bg error:', e.message));
  } catch (error) {
...
router.post('/pause-state', requireValidator, async (req, res) => {
  try {
    ...
    if (mark === undefined)
      return res.status(400).json({ message: 'mark required (0=multisig, 1=external)' });

    console.log(`📋 pauseState: proxy=${cleanProxy} mark=${mark}`);

    const result = await relay.sponsorPauseState(
      req.callerUser.safeAddress,
      privateKey,
      cleanProxy,
      mark
...
router.post('/propose-unpause', requireValidator, async (req, res) => {
  try {
    ...
    if (existing)
      return res.status(409).json({ message: 'Unpause proposal already exists for this proxy' });

    console.log(`📋 proposeUnpause: proxy=${cleanProxy} mark=${mark}`);

    const result = await relay.sponsorProposeUnpause(
      req.callerUser.safeAddress,
      privateKey,
      cleanProxy,
      mark
...
router.post('/validate-unpause', requireValidator, async (req, res) => {
  try {
    ...
    const result = await relay.sponsorValidateUnpause(
      req.callerUser.safeAddress,
      privateKey,
      cleanProxy
    );
    ...
    try {
      ...
      console.log(
        `✅ Unpause validated — remaining=${remaining}${isValidated ? ' (timelock started)' : ''}`
      );
    } catch (e) {
      console.error('❌ validate-unpause bg error:', e.message);
    }
  } catch (error) {
...
router.post('/execute-unpause', requireValidator, async (req, res) => {
  try {
    ...
    const result = await relay.sponsorExecuteUnpause(
      req.callerUser.safeAddress,
      privateKey,
      cleanProxy
    );
...
router.post('/cancel-unpause', requireValidator, async (req, res) => {
  try {
    ...
    const result = await relay.sponsorCancelUnpause(
      req.callerUser.safeAddress,
      privateKey,
      cleanProxy
    );
  ...
  try {
    ...
    if (!cleanSingleton || !cleanToken || !cleanReceiver)
      return res.status(400).json({
        message: 'singletonAddress, tokenAddress, and receiverAddress required',
      });

    console.log(`📋 withdrawFromSingleton: token=${cleanToken} receiver=${cleanReceiver}`);

    const result = await relay.sponsorWithdrawFromSingleton(
      req.callerUser.safeAddress,
      privateKey,
      cleanSingleton,
      cleanToken,
...
router.post('/update-recovery', requireValidator, async (req, res) => {
  try {
    ...
    if (typeof action !== 'boolean')
      return res.status(400).json({ message: 'action must be boolean' });

    console.log(`📋 updateRecovery: ${cleanTarget} action=${action}`);

    const result = await relay.sponsorUpdateRecovery(
      req.callerUser.safeAddress,
      privateKey,
      cleanTarget,
      action
...
router.post('/emergency-add-validator', requireValidator, async (req, res) => {
  try {
    ...
    const tx1 = await multisig.proposeValidatorUpdate(cleanTarget, true, {
      gasLimit: 500_000,
    });
    await tx1.wait();
    ...
    const tx2 = await multisig.validateValidatorUpdate(cleanTarget, {
      gasLimit: 500_000,
    });
    await tx2.wait();
    ...
    // execute
    const tx3 = await multisig.executeValidatorUpdate(cleanTarget, {
      gasLimit: 500_000,
    });
    await tx3.wait();
