...
async function debugMultisig() {
  ...
  const multisig = new ethers.Contract(MULTISIG_ADDRESS, MULTISIG_IFACE, wallet);
  ...
  try {
    ...
    await multisig.proposeInitialization.staticCall('@salva', normalizedRegistry);

    console.log('--------------------------------------------------');
    console.log('✅ RESULT: The Multisig logic is FINE.');
    console.log("This means the '@salva' namespace is available and your wallet has permission.");
    console.log('The GS013 error is happening because the SAFE rejected your signature.');
    console.log('--------------------------------------------------');
  } catch (error) {
    console.log('--------------------------------------------------');
