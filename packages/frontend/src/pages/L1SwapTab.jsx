  ...
  <div className="flex gap-2">
    {options.map((t) => (
      <button
        ...
        style={
          value === t
            ? { background: accentColor, color: "#000", borderColor: accentColor, boxShadow: `0 4px 16px ${accentColor}33` }
...
const TxStepRow = ({ active, done, label, sub }) => (
  <div className={`flex items-center gap-3 p-3 rounded-xl transition-all ${active ? "bg-blue-500/10 border border-blue-500/20" : done ? "opacity-50" : "opacity-30"}`}>
    <div className={`w-6 h-6 rounded-full flex items-center justify-center flex-shrink-0 ${done ? "bg-green-500" : active ? "bg-blue-500" : "bg-white/10"}`}>
      {done ? <span className="text-[10px] text-white">✓</span> : active ? <span className="w-3 h-3 border-2 border-white/30 border-t-white rounded-full animate-spin block" /> : <span className="w-2 h-2 rounded-full bg-white/30 block" />}
    </div>
    <div>
...
const L1SwapModal = ({ pool, section, wallet, l1Config, onClose, showMsg, onSwapComplete }) => {
  ...
  const [stableToken, setStableToken] = useState(parseFloat(pool.usdtLiquidity || 0) > 0 ? "USDT" : "USDC");
  const [ngnToken, setNgnToken] = useState(parseFloat(pool.ngnsLiquidity || 0) > 0 ? "NGNS" : "CNGN");
  ...
  const minAmount = section === "buy" ? parseFloat(pool.minNgnAmount || 0) : parseFloat(pool.minTokenAmount || 0);
  const isBelowMin = swapType === "exact_in" && amountRaw > 0 && amountRaw < minAmount;
  const displayRate = section === "buy" ? parseFloat(pool.buyRate || 0) : parseFloat(pool.sellRate || 0);

  const swapFn = (() => {
    if (section === "buy") return swapType === "exact_in" ? "swapExactNGNAmountForToken" : "swapForExactTokenAmount";
  ...
  useEffect(() => {
    ...
    fetch(`${SALVA_API_URL}/api/pool/l1/trust-status?userSafeAddress=${l1Account}&poolAddress=${pool.poolAddress}&tokenSymbol=${sym}`)
      .then((r) => r.json())
      .then((d) => { setIsTrusted(!!d.trusted); setTrustChecked(true); })
    ...
    quoteTimer.current = setTimeout(async () => {
      ...
      try {
        const res = await fetch(`${SALVA_API_URL}/api/pool/quote`, {
          ...
          body: JSON.stringify({ poolAddress: pool.poolAddress, swapFn, amount: amountRaw, isL1: true }),
        });
  ...
  const poolReceiveBal =
    tokenOut === "USDT" ? parseFloat(pool.usdtLiquidity || 0)
  ...
  const canProceed = amountRaw > 0 && !isBelowMin && trustChecked && !userCantAfford && !poolCantCover && !poolEmpty && !userBalLoading && step === "idle";

  // ── Core swap executor — runs ONCE per user intent ────────────────────────
  const executeSwap = async (doApproveMax) => {
    if (!l1Config) { showMsg("Config not loaded — please refresh", "error"); return; }
    ...
    try {
      ...
      if (!approvedAlready) {
        const currentAllowance = await tokenContract.allowance(await signer.getAddress(), poolAddr);
        approvedAlready = currentAllowance >= neededWei;
      }
      ...
      if (!approvedAlready) {
        ...
        // Uses timeout wrapper so it can't hang indefinitely
        await waitWithTimeout(approveTx, 1, 90_000);

        if (doApproveMax) {
          // Record trust in backend (fire-and-forget)
          const sym = tokenIn === "NGNS" ? "NGN" : tokenIn;
      ...
      switch (swapFn) {
        case "swapExactNGNAmountForToken":
          swapTx = await poolContract.swapExactNGNAmountForToken(...swapArgs, { gasLimit: 200_000 });
          break;
        case "swapExactTokenAmountForNGN":
          swapTx = await poolContract.swapExactTokenAmountForNGN(...swapArgs, { gasLimit: 200_000 });
          break;
  ...
  const handleContinue = () => {
    ...
    if (!isTrusted) { setShowTrust(true); return; }
    executeSwap(false);
  };

  const isProcessing = ["checking_allowance", "approving", "waiting_approve", "swapping", "waiting_swap"].includes(step);

  const inputTokenLabel = section === "buy" ? ngnLabel : stableToken;
  const outputTokenLabel = section === "buy" ? stableToken : ngnLabel;
  const amountInputLabel = swapType === "exact_in" ? `${inputTokenLabel} to spend` : `${outputTokenLabel} to receive`;
    ...
    <>
      <div className="fixed inset-0 z-[80] flex items-end sm:items-center justify-center px-0 sm:px-4">
        ...
        <motion.div
          ...
          onClick={(e) => e.stopPropagation()}
        >
          <div className="h-px" style={{ background: `linear-gradient(90deg, transparent, ${accentColor}66, transparent)` }} />
          <div className="p-6 sm:p-8">
              ...
              <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="space-y-4">
                <div className="mb-2">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    <p className="text-[9px] uppercase tracking-[0.45em] font-black" style={{ color: accentColor }}>
                      {section === "buy" ? "Buy USD Stablecoin" : "Sell USD Stablecoin"}
                    </p>
                    {isTrusted && <span className="px-2 py-0.5 rounded-full text-[9px] font-black border border-green-500/30 bg-green-500/10 text-green-400">Approved ✓</span>}
                  </div>
                  <h3 className="text-xl font-black text-white">{pool.poolName || "Anonymous Pool"}</h3>
                  <p className="font-mono text-[10px] text-white/50 truncate mt-0.5">{pool.poolAddress}</p>
                </div>

                <div>
                  <label className="text-[10px] uppercase tracking-widest text-white/60 font-black block mb-2">USD Stablecoin</label>
                  <TokenPills options={["USDT", "USDC"]} value={stableToken} onChange={setStableToken} accentColor={accentColor} />
                </div>
                <div>
                  <label className="text-[10px] uppercase tracking-widest text-white/60 font-black block mb-2">Naira Stablecoin</label>
                  <TokenPills options={["NGNS", "CNGN"]} value={ngnToken} onChange={setNgnToken} accentColor={accentColor} />
                </div>

                <div>
                  <label className="text-[10px] uppercase tracking-widest text-white/60 font-black block mb-2">Mode</label>
                  <div className="flex gap-2">
                    {[{ id: "exact_in", label: "Exact Input" }, { id: "exact_out", label: "Exact Output" }].map(({ id, label }) => (
                      <button key={id} onClick={() => { setSwapType(id); setAmountDisplay(""); setAmountRaw(0); setQuote(null); }}
                ...
                <div className="relative flex items-center gap-3 py-1">
                  ...
                  <div className="flex items-center gap-3 px-3 py-1.5 rounded-full border border-white/[0.08] bg-white/[0.03]">
                    <div className="flex items-center gap-1.5">
                      ...
                      <span className="text-[9px] font-black" style={{ color: accentColor }}>{section === "buy" ? ngnLabel : stableToken}</span>
                    </div>
                    <span className="text-white/20 text-[9px]">·</span>
                    <div className="flex items-center gap-1.5">
                      <span className="text-[9px] text-green-400">↓</span>
                      <span className="text-[9px] uppercase tracking-[0.25em] font-black text-white/40">Receive</span>
                      <span className="text-[9px] font-black text-green-400">{section === "buy" ? stableToken : ngnLabel}</span>
                    </div>
                  </div>
                  <div className="flex-1 h-px bg-gradient-to-r from-transparent via-white/10 to-transparent" />
                </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="px-3 py-2.5 rounded-xl border border-white/[0.06] bg-white/[0.02]">
                    ...
                    {userBalLoading
                      ...
                      : <p className={`text-xs font-black ${userCantAfford ? "text-red-400" : "text-white"}`}>{userSendBal !== null ? fmt(userSendBal) : "—"} <span className="font-normal opacity-60">{section === "buy" ? ngnLabel : stableToken}</span></p>
                    }
                  </div>
                  <div className={`px-3 py-2.5 rounded-xl border ${poolEmpty ? "border-red-500/30 bg-red-500/5" : "border-white/[0.06] bg-white/[0.02]"}`}>
                    ...
                    <p className={`text-xs font-black ${poolEmpty || poolCantCover ? "text-red-400" : "text-green-400"}`}>{fmt(poolReceiveBal)} <span className="font-normal opacity-60">{section === "buy" ? stableToken : ngnLabel}</span></p>
                  </div>
                </div>

                {userCantAfford && <p className="text-[10px] text-red-400 font-bold -mt-2">⚠ Insufficient balance</p>}
                {(poolEmpty || poolCantCover) && (
                  <p className="text-[10px] text-red-400 font-bold -mt-2">
                    {poolEmpty ? `⚠ No ${section === "buy" ? stableToken : ngnLabel} liquidity in pool` : `⚠ Pool only has ${fmt(poolReceiveBal)} ${section === "buy" ? stableToken : ngnLabel}`}
                  </p>
                )}

                <div>
                  <label className="text-[10px] uppercase tracking-widest text-white/60 font-black block mb-2">{amountInputLabel}</label>
                  <div className="relative">
                    <input
                      ...
                      placeholder="0.00"
                      value={amountDisplay}
                      onChange={(e) => { const f = fmtInput(e.target.value); setAmountDisplay(f); setAmountRaw(parseFloat(f.replace(/,/g, "")) || 0); }}
                    ...
                    />
                    <span className="absolute right-4 top-1/2 -translate-y-1/2 font-black text-sm" style={{ color: accentColor }}>{amountInputSuffix}</span>
                  </div>
                  {isBelowMin && (
                    <p className="text-[11px] text-red-400 font-bold mt-1.5 animate-pulse">
                      ⚠️ Minimum: {section === "buy" ? `${fmt(minAmount, 0)} ${ngnLabel}` : `${fmt(minAmount)} ${stableToken}`}
                    </p>
                  )}
                </div>
                ...
                {(quote !== null || quoteLoading) && amountRaw > 0 && (
                  <div className="flex items-center justify-between p-4 rounded-xl bg-white/5 border border-white/10">
                    <span className="text-[10px] uppercase tracking-widest text-white/60 font-black">{quoteLabel}</span>
                    {quoteLoading
                      ? <span className="w-4 h-4 border-2 border-blue-400/30 border-t-blue-400 rounded-full animate-spin" />
                      : <span className="font-black text-sm" style={{ color: accentColor }}>{fmt(quote)} {quoteSuffix}</span>
                    }
                  </div>
                )}

                <div className="flex items-center justify-between p-4 rounded-xl bg-white/[0.03] border border-white/[0.06]">
                  <span className="text-[10px] uppercase tracking-widest text-white/60 font-black">Exchange Rate</span>
                  <span className="font-black text-sm text-white">₦{fmt(displayRate, 0)}<span className="text-white/60 font-normal text-xs"> / USD</span></span>
                </div>

                <div className="p-3 rounded-xl bg-blue-500/5 border border-blue-500/15">
                  <p className="text-[10px] text-blue-400/80 font-bold leading-relaxed">
                    ⚡ Your wallet signs all transactions.{!isTrusted && " You may need to approve spending once before swapping."}
                  </p>
                </div>

                <div className="flex gap-3 pt-1">
                  <button onClick={onClose} className="flex-1 py-3.5 rounded-xl border border-white/10 text-white font-bold text-sm hover:bg-white/5 transition-all">Cancel</button>
                  <button
                    ...
                    style={{ background: accentColor, color: "#000", boxShadow: `0 8px 24px ${accentColor}33` }}
                  >
            ...
            {isProcessing && (
              <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} className="py-6 space-y-5">
                <div className="text-center mb-2">
                  ...
                  <p className="text-xs text-white/60 mt-1 max-w-[260px] mx-auto leading-relaxed">{stepMsg}</p>
                </div>
                ...
                <div className="space-y-2">
                  <TxStepRow
                    ...
                    sub={step === "waiting_approve" ? "Confirming on-chain…" : step === "approving" ? "Check your wallet" : isTrusted ? "Already approved" : ""}
                  />
                  <TxStepRow
                    active={["swapping", "waiting_swap"].includes(step)}
                    done={false}
                    label="Execute Swap"
                    sub={step === "waiting_swap" ? "Confirming on-chain…" : step === "swapping" ? "Check your wallet" : ""}
                  />
                </div>

                {txHash && (
                  <a href={`https://${process.env.NODE_ENV === "production" ? "" : "sepolia."}etherscan.io/tx/${txHash}`}
              ...
              <div className="text-center py-8">
                ...
                {receivedAmount !== null && (
                  <p className="text-sm text-white/60 mb-4">
                    You received <span className="font-black text-white">{fmt(receivedAmount)}</span>{" "}
                    <span className="font-black" style={{ color: accentColor }}>{receivedToken}</span>
                  </p>
                )}
                {txHash && (
                  <a href={`https://${process.env.NODE_ENV === "production" ? "" : "sepolia."}etherscan.io/tx/${txHash}`}
                ...
                <button onClick={onClose} className="w-full mt-5 py-3.5 rounded-xl bg-blue-500 text-white font-black text-sm hover:brightness-110 shadow-lg shadow-blue-500/20 transition-all">
                  Done
                </button>
              </div>
            )}
            ...
            {step === "error" && (
              <div className="text-center py-8">
                ...
                <p className="text-xs text-red-400 mb-6 leading-relaxed max-w-[280px] mx-auto">{swapError}</p>
                <div className="flex gap-3">
                  <button onClick={onClose} className="flex-1 py-3.5 rounded-xl border border-white/10 text-white font-bold text-sm hover:bg-white/5 transition-all">Close</button>
                  <button onClick={() => { setStep("idle"); setSwapError(null); }} className="flex-1 py-3.5 rounded-xl bg-blue-500 text-white font-black text-sm hover:brightness-110 transition-all">Try Again</button>
                </div>
              </div>
            )}
          </div>
      ...
      <AnimatePresence>
        {showTrust && (
          <TrustModal
            ...
            onTrust={() => { setShowTrust(false); executeSwap(true); }}
            onSkip={() => { setShowTrust(false); executeSwap(false); }}
      ...
      <div className="p-5">
        <div className="flex items-start justify-between gap-3 mb-4">
          <div className="min-w-0">
            <p className="font-black text-base text-white truncate">{pool.poolName || "Anonymous Pool"}</p>
            <p className="font-mono text-[10px] text-white/60 truncate mt-0.5">{pool.poolAddress}</p>
          </div>
          <div className="flex-shrink-0 px-2.5 py-1 rounded-lg border text-[9px] font-black uppercase tracking-widest"
        ...
        {section === "buy" ? (
          <div className="grid grid-cols-3 gap-2 mb-4">
            <div className="p-3 rounded-xl bg-white/[0.04] border border-white/[0.06]">
              <p className="text-[9px] uppercase tracking-[0.3em] text-white/60 font-black mb-1">Rate</p>
              <p className="font-black text-sm text-salvaGold">₦{fmt(rate, 0)}<span className="text-[10px] text-white/60 font-normal">/USD</span></p>
              {parseFloat(pool.minNgnAmount || 0) > 0 && <p className="text-[9px] text-yellow-400/70 mt-0.5 font-bold">Min: {fmt(parseFloat(pool.minNgnAmount), 0)}</p>}
            </div>
            <div className="p-3 rounded-xl bg-white/[0.04] border border-white/[0.06]">
              <p className="text-[9px] uppercase tracking-[0.3em] text-green-400/50 font-black mb-1">USDT</p>
              <p className="font-black text-sm text-green-400">${fmt(usdtAvail)}</p>
            </div>
            <div className="p-3 rounded-xl bg-white/[0.04] border border-white/[0.06]">
              <p className="text-[9px] uppercase tracking-[0.3em] text-blue-400/50 font-black mb-1">USDC</p>
          ...
          <div className="grid grid-cols-3 gap-2 mb-4">
            <div className="p-3 rounded-xl bg-white/[0.04] border border-white/[0.06]">
              <p className="text-[9px] uppercase tracking-[0.3em] text-white/60 font-black mb-1">Rate</p>
              <p className="font-black text-sm text-green-400">₦{fmt(rate, 0)}<span className="text-[10px] text-white/60 font-normal">/USD</span></p>
              {parseFloat(pool.minTokenAmount || 0) > 0 && <p className="text-[9px] text-yellow-400/70 mt-0.5 font-bold">Min: ${fmt(parseFloat(pool.minTokenAmount))}</p>}
            </div>
            <div className="p-3 rounded-xl bg-white/[0.04] border border-white/[0.06]">
              <p className="text-[9px] uppercase tracking-[0.3em] text-salvaGold/50 font-black mb-1">NGNs</p>
              <p className="font-black text-sm text-salvaGold">{fmt(ngnsAvail, 0)}</p>
            </div>
            <div className="p-3 rounded-xl bg-white/[0.04] border border-white/[0.06]">
              <p className="text-[9px] uppercase tracking-[0.3em] text-white/60 font-black mb-1">cNGN</p>
        ...
        <button
          ...
          style={{ background: accentColor, color: "#000", boxShadow: `0 4px 16px ${accentColor}33` }}
        >
          Proceed to Swap →
        </button>
      </div>
...
const L1SwapTab = ({ l1Config, configLoading, showMsg }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-[9px] uppercase tracking-[0.45em] text-blue-400/60 font-black mb-1">Salva V3 · Ethereum Chain</p>
          <h2 className="text-3xl font-black tracking-tight">Naira Exchange</h2>
        </div>
        <div className="flex items-center gap-2 flex-shrink-0 mt-1">
          <a href="/dashboard" className="flex items-center gap-1 px-2.5 py-1.5 rounded-xl border border-salvaGold/30 bg-salvaGold/[0.07] hover:bg-salvaGold/[0.14] transition-all">
          ...
          {lastTime && <p className="text-[9px] text-white/60 font-bold uppercase tracking-widest hidden sm:block">{lastTime.toLocaleTimeString()}</p>}
          <button onClick={() => fetchPools(true)} disabled={refreshing}
            ...
            {refreshing ? <span className="w-4 h-4 border-2 border-blue-400/30 border-t-blue-400 rounded-full animate-spin" /> : <span className="text-blue-400 text-lg leading-none">↻</span>}
          </button>
        </div>
      </div>
  ...
  />
      ...
      {/* Info */}
      <div className="p-3 rounded-xl bg-blue-500/5 border border-blue-500/15">
        <p className="text-[10px] text-blue-400/80 font-bold">⚡ Your external wallet signs swap transactions directly.</p>
      </div>

      {/* Search */}
      <div className="relative">
        <svg className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-white/60" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        />
        {search && (
          <button onClick={() => setSearch("")} className="absolute right-4 top-1/2 -translate-y-1/2 text-white/60 hover:text-white/80 transition-colors text-xs font-black">✕</button>
        )}
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {[
          { id: "buy", label: "NGN → USD", sub: "Spend NGN, get USD", count: buyPools.length, color: "#D4AF37" },
          { id: "sell", label: "USD → NGN", sub: "Spend USD, get NGN", count: sellPools.length, color: "#22c55e" },
          ...
          <button key={id} onClick={() => setSection(id)}
            ...
            <div className="flex items-center justify-between mb-0.5">
              <span className="font-black text-sm" style={{ color: section === id ? color : "rgba(255,255,255,0.5)" }}>{label}</span>
              <span className="text-[9px] font-black px-1.5 py-0.5 rounded-md"
                style={section === id ? { background: `${color}20`, color } : { background: "rgba(255,255,255,0.05)", color: "rgba(255,255,255,0.25)" }}>
                {count}
              </span>
            </div>
      ...
      {loading ? (
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-20 rounded-3xl border border-dashed border-white/[0.06] text-center">
          <div className="w-14 h-14 bg-white/5 border border-white/10 rounded-2xl flex items-center justify-center mx-auto mb-4">
            <span className="text-2xl">🏊</span>
          </div>
          <p className="font-black text-white/60 text-sm">{search ? "No pools match your search." : "No active pools in this section."}</p>
          {search && <button onClick={() => setSearch("")} className="mt-3 text-[10px] font-black text-blue-400/60 hover:text-blue-400 uppercase tracking-widest transition-colors">Clear search</button>}
        </motion.div>
      ) : (
