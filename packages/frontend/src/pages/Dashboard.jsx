...
const Dashboard = () => {
  ...
  const [aliasStatus, setAliasStatus] = useState(() => {
    ...
    return { hasName: false, hasNumber: false, nameAlias: null, numberAlias: null };
  });
  ...
  const resolveAndConfirm = async () => {
    ...
    try {
      ...
      if (to.startsWith("0x")) {
        ...
        const nsDisplay = selectedRegistry?.name?.replace(/\s+/g, "").toLowerCase() || "salva";
  ...
  const AliasModal = () => {
    ...
    return (
      <div className="fixed inset-0 z-[60] flex items-center justify-center px-4">
        <motion.div onClick={() => setShowAliasModal(false)} className="absolute inset-0 bg-black/95 backdrop-blur-md"
  ...
  return (
    <div className="min-h-screen bg-white dark:bg-[#0A0A0B] text-black dark:text-white pt-24 px-4 pb-12 relative overflow-x-hidden">
      ...
      <div className="max-w-4xl mx-auto relative z-10">
        ...
        {!bothAliasLinked && (
          <motion.button onClick={() => setShowAliasModal(true)} whileHover={{ scale: 1.01 }} whileTap={{ scale: 0.99 }}
                ...
                <div className="space-y-2">
                  ...
                  <input required type="text" placeholder="Name, account number, or 0x address"
                    ...
                    onChange={(e) => {
                      ...
                      if (val.trim().length > 0 && !val.trim().startsWith("0x")) {
      ...
      <AnimatePresence>
        {isConfirmModalOpen && confirmationData && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center px-4">
            <motion.div onClick={() => setIsConfirmModalOpen(false)} className="absolute inset-0 bg-black/95 backdrop-blur-md"
              initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} />
            <motion.div onClick={(e) => e.stopPropagation()}
              className="relative bg-white dark:bg-zinc-900 p-8 rounded-3xl w-full max-w-md border border-gray-200 dark:border-white/10 shadow-2xl"
              initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}>
              ...
              <div className="flex gap-3">
                ...
                <button onClick={() => { setIsConfirmModalOpen(false); setIsPinModalOpen(true); setTransactionPin(""); setPinAttempts(0); }}
      ...
      <AnimatePresence>
        {isPinModalOpen && (
          <div className="fixed inset-0 z-[70] flex items-center justify-center px-4">
            <motion.div onClick={() => !loading && setIsPinModalOpen(false)} className="absolute inset-0 bg-black/95 backdrop-blur-md"
              initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} />
            <motion.div onClick={(e) => e.stopPropagation()}
              className="relative bg-white dark:bg-zinc-900 p-8 rounded-3xl w-full max-w-md border border-gray-200 dark:border-white/10 shadow-2xl"
              initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}>
              ...
              {pinAttempts > 0 && (
                <p className="text-xs text-red-500 text-center mb-4 font-bold">⚠️ {3 - pinAttempts} attempts remaining</p>
              )}
              <div className="flex gap-3">
                ...
                <button onClick={verifyPinAndProceed} disabled={loading || transactionPin.length !== 4}
