...
// Mirror of the rewritten DeployPool.jsx, adapted for BNB Chain.
import React, { useState, useEffect, useCallback } from 'react';
import { motion, AnimatePresence } from 'framer-motion';
import { SALVA_API_URL } from '../config';

// ─── Chain constants ──────────────────────────────────────────────────────────
// This is the BNB build — mirrors DeployPool.jsx (Base) with CHAIN/'bnb',
// BALANCE_PREFIX='bnb', and gold swapped for blue.
//

const CHAIN = 'bnb';
const BALANCE_PREFIX = 'bnb';
...
// Single shared PIN-verify helper for this file — BNB verify endpoint.
const verifyBnbPin = async (email, pin) => {
      ...
      >
        {value ? (
          <div className="flex items-center gap-2 sm:gap-3 min-w-0 flex-1">
            <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-lg bg-blue-500/20 border border-blue-500/20 flex items-center justify-center flex-shrink-0">
              <span className="text-blue-400 text-[9px] sm:text-xs font-black">
              ...
              {filtered.length === 0 ? (
                ...
                filtered.map((reg) => (
                  <button
                    ...
                    className={`w-full flex items-center gap-2 sm:gap-3 px-3 py-2 sm:px-4 sm:py-3 hover:bg-blue-500/5 transition-colors text-left ${
                      value?.registryAddress === reg.registryAddress ? 'bg-blue-500/10' : ''
                    }`}
                  >
                    <div className="w-6 h-6 sm:w-8 sm:h-8 rounded-lg bg-blue-500/15 border border-blue-500/20 flex items-center justify-center flex-shrink-0">
                      <span className="text-blue-400 text-[9px] sm:text-xs font-black">
                    ...
                    {value?.registryAddress === reg.registryAddress && (
                      <span className="text-blue-400 text-[10px] sm:text-sm">✓</span>
                    )}
                  </button>
                ))
              )}
            ...
            <div className="-mt-2 mb-4 sm:-mt-3 sm:mb-6 px-2.5 py-2 sm:px-3 sm:py-2.5 rounded-xl bg-white/[0.03] border border-white/[0.06] flex items-center justify-between text-[9px] sm:text-xs">
              ...
              {feeInfo.loading ? (
                ...
                <span className="text-red-400 font-black">
                  ...
                  {feeInfo.feeUsd != null ? ` (~$${smartFmt(feeInfo.feeUsd)})` : ''}
                </span>
              ) : (
...
const PoolManagePanel = ({ pool, user, showMsg, onClose, onRefresh }) => {
  ...
  // Only liquidity provide/remove estimates a fee — same as Base build.
  const fetchLiquidityFee = useCallback((type) => {
    setPanelFee({ feeNGN: null, feeUsd: null, loading: true });
    fetch(`${SALVA_API_URL}/api/user/estimate-provide-remove-liquidity-fee/${CHAIN}/${type}`)
      .then((r) => r.json())
      .then((d) =>
        setPanelFee({
  ...
  const verifyPin = async (pin) => {
    setPinLoading(true);
    try {
      const { ok, data } = await verifyBnbPin(user.email, pin);
      if (!ok) {
      ...
      else if (pinAction === 'buyRate') await executeUpdateRate(pkey, 'buy', buyRate);
      else if (pinAction === 'sellRate') await executeUpdateRate(pkey, 'sell', sellRate);
      else if (pinAction === 'pause') await executeUpdateState(pkey, 'pause');
      else if (pinAction === 'unpause') await executeUpdateState(pkey, 'unpause');
    } catch {
  ...
  const executeLiquidity = async (pkey, type) => {
    ...
    try {
      ...
      let res;
      try {
        res = await fetch(`${SALVA_API_URL}/api/pool/liquidity`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
      ...
      if (err.name === 'AbortError') {
        ...
        showMsg(err.message || 'Liquidity transaction failed.', 'error');
      }
    } finally {
      setTxLoading(false);
    }
      ...
      >
        ...
        <div className="px-3 pt-2.5 pb-2.5 sm:px-4 sm:pt-3 sm:pb-3 border-b border-white/[0.05]">
          <div className="flex items-start justify-between gap-2 mb-2.5 sm:mb-3">
            <div className="min-w-0">
              <p className="text-[7px] sm:text-[9px] uppercase tracking-[0.35em] text-blue-400/60 font-black mb-0.5">
          ...
          <div className="rounded-xl border border-white/[0.06] bg-white/[0.02] overflow-hidden divide-y divide-white/[0.05]">
            <div className="flex items-center justify-between px-2.5 py-1.5 sm:px-3 sm:py-2">
              <span className="text-[7px] sm:text-[9px] uppercase tracking-widest text-blue-400/60 font-black">
                NGN Total
              </span>
              <span className="font-black text-[10px] sm:text-sm text-blue-400 tabular-nums">
                {compactFmt(totalNgn)}
              </span>
            </div>
            <div className="flex items-center justify-between px-2.5 py-1.5 sm:px-3 sm:py-2">
              ...
              <div className="rounded-2xl border border-blue-500/20 bg-blue-500/[0.02] overflow-hidden">
                ...
                <div className="p-2.5 sm:p-3.5 space-y-1.5 sm:space-y-2.5">
                  ...
                  <button
                    ...
                    className="w-full py-2 sm:py-3 rounded-xl font-black text-[9px] sm:text-xs uppercase tracking-widest transition-all disabled:opacity-40 flex items-center justify-center gap-1.5 sm:gap-2 bg-blue-500/10 border border-blue-500/25 text-blue-400 hover:bg-blue-500 hover:text-white hover:border-blue-500"
                  >
        ...
        <AnimatePresence>
          {pinVisible && (
            <PinModal
              ...
              loading={pinLoading}
              feeInfo={pinAction === 'provide' || pinAction === 'remove' ? panelFee : undefined}
            />
          )}
        </AnimatePresence>
      </motion.div>
    ...
    >
      ...
      <div className="p-2.5 sm:p-3.5 space-y-2 sm:space-y-3">
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            <div className="flex items-center gap-1 sm:gap-1.5 flex-wrap mb-0.5">
              <p className="font-black text-blue-400 text-[10px] sm:text-sm truncate">
                {pool.poolName || 'Unnamed Pool'}
              </p>
              <SubBadge pool={pool} />
            </div>
        ...
        <div className="flex gap-1 sm:gap-1.5">
          ...
          <button
            onClick={onPublish}
            className="flex-1 py-1.5 sm:py-2 rounded-lg bg-blue-500 text-white font-black text-[7px] sm:text-[10px] uppercase tracking-widest hover:brightness-110 transition-all shadow-lg shadow-blue-500/20"
          >
            {pool.isSubscribed ? 'Extend' : 'Publish'}
          </button>
          <button
            onClick={onName}
            className="py-1.5 px-1.5 sm:py-2 sm:px-2.5 rounded-lg border border-blue-500/25 text-blue-400 font-black text-[7px] sm:text-[10px] uppercase hover:bg-blue-500/10 transition-all"
          >
            {pool.poolName ? '✎' : 'Name'}
          </button>
          <button
...
const PoolNameModal = ({
  ...
  const [registry, setRegistry] = useState(registries.length === 1 ? registries[0] : null);
  ...
  const handleCheck = async () => {
    ...
    if (!registry) return setNameError('Select which wallet service this name belongs to');
    setNameError('');
    setChecking(true);
    try {
      ...
      if (existingName) {
        jobs.push(
          fetch(`${SALVA_API_URL}/api/pool/single-pool/${poolAddress}/${CHAIN}`)
            .then((r) => r.json())
            .then((d) => setOldRegistryAddress(d?.pool?.registryAddress || null))
  ...
  const handleConfirmPin = async (pin) => {
    ...
    setFinalError('');
    try {
      const { ok, data: verifyData } = await verifyBnbPin(user.email, pin);
      if (!ok) {
        showMsg('Invalid PIN', 'error');
        setPinLoading(false);
        return;
      ...
      if (!linkRes.ok || !linkData.status) {
        if (existingName) {
          setFinalError('The new name could not be linked. You can retry from here.');
          setStep('confirm');
        } else {
          setFinalError(
    ...
    <div className="fixed inset-0 z-[85] flex items-end sm:items-center justify-center px-0 sm:px-4">
      ...
      >
        ...
        <div className="px-4 pt-4 pb-3 sm:px-6 sm:pt-5 sm:pb-4 border-b border-white/[0.05] flex items-center justify-between">
          <div>
            <p className="text-[7px] sm:text-[9px] uppercase tracking-[0.45em] text-blue-400/60 font-black mb-0.5">
              Salva NS
            </p>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-3.5 sm:space-y-5">
          {existingName && (
            <div className="flex items-center gap-2 sm:gap-3 p-2.5 sm:p-3.5 rounded-xl bg-white/[0.03] border border-white/[0.06]">
              ...
              <span className="text-blue-400 font-black text-[10px] sm:text-sm truncate flex-1">
                {existingName}
              </span>
          ...
          {step === 'form' && (
            ...
            >
              <div>
                ...
                />
                {nameInput && registry && (
                  <p className="text-[7px] sm:text-[10px] text-blue-400/60 font-bold mt-1 sm:mt-1.5 ml-1">
                    Preview: {nameInput}
                    {registry.nspace}
                  </p>
                )}
              ...
              <button
                ...
                className="w-full py-2.5 sm:py-4 bg-blue-500 text-white font-black rounded-xl hover:brightness-110 transition-all disabled:opacity-40 uppercase tracking-widest text-[10px] sm:text-sm flex items-center justify-center gap-1.5 sm:gap-2 shadow-lg shadow-blue-500/20"
              >
                {checking && (
                  <span className="w-2.5 h-2.5 sm:w-4 sm:h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />
                )}
              ...
              <div className="flex items-start gap-2.5 sm:gap-4">
                ...
                <div>
                  ...
                  <p className="text-[8px] sm:text-[11px] text-white/60 mt-0.5 leading-relaxed">
                    <span className="text-blue-400 font-black">{weldedName}</span> is reserved.
          ...
          {step === 'confirm' && (
            <motion.div
              ...
              className="space-y-2.5 sm:space-y-4"
            >
              <div className="p-3.5 sm:p-5 rounded-2xl bg-blue-500/8 border border-blue-500/20 text-center">
                <p className="text-[7px] sm:text-[9px] uppercase tracking-[0.3em] font-black text-blue-400/50 mb-1.5 sm:mb-2">
                  Name Available
                </p>
                <p className="text-base sm:text-2xl font-black text-blue-400">{weldedName}</p>
              </div>
              {linkFeeLoading ? (
                <div className="flex items-center gap-2 sm:gap-3 p-2.5 sm:p-4 rounded-xl bg-white/5 border border-white/10">
                  ...
                  <p className="text-[9px] sm:text-xs text-blue-400 font-bold">Fetching fees…</p>
                </div>
              ) : (
                <>
                  {linkFee !== null && Number(linkFee) > 0 ? (
                    <div className="flex items-center justify-between p-2.5 sm:p-4 rounded-xl bg-white/5 border border-white/10">
                      ...
                      <p className="font-black text-white text-[10px] sm:text-sm">
                        ...
                        <span className="text-blue-400 text-[8px] sm:text-xs">NGNs</span>
                      </p>
                    </div>
                  ) : (
              ...
              {existingName && (
                <div className="p-2.5 sm:p-4 rounded-xl bg-yellow-500/5 border border-yellow-500/15">
                  ...
                  <p className="text-[9px] sm:text-xs text-white/60 leading-relaxed">
                    1. <span className="text-red-400 font-black">{existingName}</span> unlinked
                    on-chain
                    <br />
                    2. <span className="text-blue-400 font-black">{weldedName}</span> linked to this
                    pool
                  </p>
                </div>
              )}
              {finalError && (
                <div className="flex items-center gap-1.5 sm:gap-2 px-2 py-1.5 sm:px-3 sm:py-2.5 rounded-xl bg-red-500/8 border border-red-500/20">
                  ...
                  <p className="text-[9px] sm:text-xs text-red-400 font-bold">{finalError}</p>
                </div>
              )}
              <div className="flex gap-2 sm:gap-3 pt-0.5 sm:pt-1">
                ...
                <button
                  ...
                  className="flex-1 py-2.5 sm:py-3.5 rounded-xl bg-blue-500 text-white font-black text-[10px] sm:text-sm hover:brightness-110 disabled:opacity-50 transition-all shadow-lg shadow-blue-500/20"
                >
                  Continue & Enter PIN
                </button>
              </div>
          ...
          {step === 'linking' && (
            ...
            >
              ...
              <p className="text-[9px] sm:text-xs text-white/60">
                {existingName ? 'Unlinking old, linking new' : 'Broadcasting to Base Chain'} · 30–90
                seconds
              </p>
            </motion.div>
          )}
          ...
          {step === 'success' && (
            ...
            >
              ...
              <div>
                <p className="text-base sm:text-xl font-black text-white">{weldedName}</p>
                <p className="text-[8px] sm:text-[11px] text-white/60 mt-0.5 sm:mt-1">
                  Your pool name is now live on Base Chain
                </p>
              </div>
              <button
                onClick={() => {
                ...
                className="w-full py-2.5 sm:py-4 bg-blue-500 text-white font-black rounded-2xl hover:brightness-110 transition-all shadow-lg shadow-blue-500/20 uppercase tracking-widest text-[10px] sm:text-sm"
              >
                Done
              </button>
            </motion.div>
...
// ─── Main BNBDeployPool ───────────────────────────────────────────────────────
const BNBDeployPool = ({ user, showMsg }) => {
  ...
  const [namingPool, setNamingPool] = useState(null);
  ...
  const fetchPoolFullData = useCallback(async (basePool) => {
    const [balRes, rateRes, subRes, pauseRes] = await Promise.all([
      fetch(`${SALVA_API_URL}/api/user/${BALANCE_PREFIX}/balance/${basePool.poolAddress}`)
        .then((r) => r.json())
        .catch(() => ({})),
      fetch(`${SALVA_API_URL}/api/pool/rate/${basePool.poolAddress}/${CHAIN}`)
        .then((r) => r.json())
        .catch(() => ({})),
      fetch(`${SALVA_API_URL}/api/pool/subscription-status/${basePool.poolAddress}/${CHAIN}`)
        .then((r) => r.json())
        .catch(() => ({})),
      fetch(`${SALVA_API_URL}/api/pool/isPaused/${basePool.poolAddress}/${CHAIN}`)
        .then((r) => r.json())
        .catch(() => ({})),
    ]);
    return {
  ...
  const fetchMyPools = useCallback(
    async (silent = false) => {
      ...
      try {
        const res = await fetch(`${SALVA_API_URL}/api/pool/pools/${user.safeAddress}/${CHAIN}`);
        const data = await res.json();
        const basePools = data.pools || [];
        const enriched = await Promise.all(basePools.map((p) => fetchPoolFullData(p)));
        setPools(enriched);
  ...
  const refreshOnePool = useCallback(
    async (poolAddress) => {
      ...
      setPools((prev) => prev.map((p) => (p.poolAddress === poolAddress ? fresh : p)));
      return fresh;
    },
  ...
  const executeDeploy = async (pin) => {
    ...
    try {
      ...
      const res = await fetch(`${SALVA_API_URL}/api/pool/deploy`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: user.email, pin: verifyData.privateKey, chain: CHAIN }),
      });
      const data = await res.json();
      if (!res.ok || !data.status) throw new Error(data.message || 'Deploy failed');
  ...
  const executeSubscribe = async (pin) => {
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok || !data.status) throw new Error(data.message || 'Subscription failed');
      showMsg(
        `Pool published! Expires ${new Date(data.expiresAt).toLocaleDateString('en-US', {
          day: 'numeric',
          month: 'short',
          year: 'numeric',
        })}`
    ...
    >
      <div className="flex items-center justify-between gap-2 sm:gap-3">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0 mt-1">
          <a
            href="/dashboard"
            className="flex items-center gap-1 px-2 py-1 sm:px-2.5 sm:py-1.5 rounded-xl border border-salvaGold/30 bg-salvaGold/[0.07] hover:bg-salvaGold/[0.14] hover:border-salvaGold/50 transition-all"
          >
          ...
          <button
            ...
            disabled={deploying}
            className="flex items-center gap-1.5 sm:gap-2 px-3.5 py-2 sm:px-5 sm:py-3 bg-blue-500 text-white font-black text-[9px] sm:text-xs uppercase tracking-widest rounded-xl hover:brightness-110 transition-all disabled:opacity-50 shadow-lg shadow-blue-500/20"
          >
            {deploying && (
          ...
          <div className="fixed inset-0 z-[80] flex items-center justify-center px-3 sm:px-4">
            ...
            >
              ...
              <div className="p-5 sm:p-8">
                <p className="text-[7px] sm:text-[9px] uppercase tracking-[0.45em] text-blue-400/60 font-black mb-1">
                  Marketplace
                </p>
                ...
                <p className="text-[9px] sm:text-xs text-white/60 mb-3.5 sm:mb-5 leading-relaxed">
                  {SUBSCRIPTION_MONTHLY_FEE_NGN.toLocaleString()} NGN per month. Choose how many
                  months to {selectedPool.isSubscribed ? 'extend' : 'subscribe'} for.
                </p>
                <div className="mb-3.5 sm:mb-5">
                  ...
                  <input
                    ...
                    className={darkInput}
                  />
                  <p className="text-[7px] sm:text-[10px] text-blue-400/70 font-bold mt-1.5 ml-1">
                    Total:{' '}
                    {(
                      SUBSCRIPTION_MONTHLY_FEE_NGN * (parseInt(subInterval, 10) || 1)
                    ).toLocaleString()}{' '}
                    NGNs
                  </p>
                </div>
                <div className="flex gap-2 sm:gap-3">
                  ...
                  <button
                    ...
                    className="flex-1 py-2.5 sm:py-3.5 rounded-xl bg-blue-500 text-white font-black text-xs sm:text-sm hover:brightness-110 disabled:opacity-50 shadow-lg shadow-blue-500/20 transition-all"
                  >
                    Subscribe
                  </button>
                </div>
            ...
            >
              ...
              <div className="p-5 sm:p-8">
                ...
                <p className="text-[9px] sm:text-xs text-white/60 mb-3 sm:mb-4 leading-relaxed">
                  Removes this pool from your dashboard. The contract stays on-chain.
                </p>
                {deletingPool.poolName && (
                  <div className="mb-3.5 sm:mb-5 px-3 py-2 sm:px-4 sm:py-3 rounded-xl bg-yellow-500/5 border border-yellow-500/20">
                    ...
                    <p className="text-[9px] sm:text-xs text-white/60 leading-relaxed">
                      <span className="text-blue-400 font-black">{deletingPool.poolName}</span> will
                      be automatically unlinked.
                    </p>
                  </div>
                )}
                <div className="flex gap-2 sm:gap-3">
