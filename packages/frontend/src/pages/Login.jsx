...
const Login = () => {
  ...
  const handleStartRegistration = async (e) => {
    ...
    try {
      ...
      if (res.ok) { setRegStep(2); showMsg('Verification code sent!'); }
      else { const d = await res.json(); showMsg(d.message || 'Failed to send code', 'error'); }
    } catch (_) { showMsg('Backend offline', 'error'); }
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isLogin && regStep === 2) {
      ...
      } catch (_) { setLoading(false); return showMsg('Verification error', 'error'); }
    }
    ...
    try {
      ...
      if (response.ok) {
        if (!data.safeAddress || !data.ownerPrivateKey) throw new Error('Invalid response from server');

        const userData = {
          username: sanitizeInput(data.username),
          email: sanitizeInput(formData.email),
          safeAddress: data.safeAddress,
          // accountNumber is no longer set at registration — user registers alias later
        ...
        // Check PIN status
        try {
          const pinRes = await fetch(`${SALVA_API_URL}/api/user/pin-status/${encodeURIComponent(sanitizeInput(formData.email))}`);
  ...
  return (
    <div className="relative min-h-screen flex items-center justify-center overflow-hidden bg-white dark:bg-[#0A0A0B] transition-colors duration-500">
      ...
      <AnimatePresence>
        {notif.show && (
          <motion.div initial={{ x: 400 }} animate={{ x: 0 }} exit={{ x: 400 }}
            className={`fixed top-10 right-4 sm:right-10 z-[100] p-5 rounded-2xl border shadow-2xl max-w-xs sm:max-w-sm ${notif.type === 'error' ? 'bg-red-500/20 border-red-500' : 'bg-zinc-900 border-salvaGold'}`}>
      ...
      <motion.div
        ...
        className="z-10 w-full max-w-md mx-4 p-8 sm:p-10 rounded-[2.5rem] border border-gray-200 dark:border-white/10 bg-white/90 dark:bg-black/40 backdrop-blur-2xl shadow-2xl"
      >
              ...
              <div className="space-y-2">
                ...
                {isLogin && (
                  <div className="flex justify-end px-1">
                    <Link to="/forgot-password" className="text-[10px] uppercase text-salvaGold/60 hover:text-salvaGold transition-colors font-bold tracking-widest">
                      Forgot Password?
                    </Link>
                  </div>
                )}
        ...
        <div className="mt-6 flex flex-col items-center gap-4">
          <button onClick={() => { setIsLogin(!isLogin); setRegStep(1); setOtp(''); }}
