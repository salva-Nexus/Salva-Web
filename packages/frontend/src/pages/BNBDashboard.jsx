...
const LinkNameTab = ({ user, registries, showMsg, onSwitchToBuy }) => {
  ...
  // ── Gas-reimbursement fee state — separate from the singleton registry
  // fee above. Linking/unlinking always executes on the Base Safe (the
  // registry lives on Base), so this fee is computed/collected there too.
  ...
  const handleExecuteLink = async () => {
    if (pinInput.length !== 4 || !preparedLinkData) return;
    setPinLoading(true);
    try {
      const baseUser = (() => {
        try {
      ...
      setLinkStep('linking');
      // Already prepared at "Check Availability" — reuse it directly.
      const prepData = preparedLinkData;
      ...
      {linkStep === 'confirm' && nameCheckResult && (
        ...
        >
          ...
          <div className="p-3.5 sm:p-5 rounded-2xl bg-blue-500/6 border border-blue-500/20 text-center">
            ...
            <p className="text-base sm:text-2xl font-black text-blue-500/">{nameCheckResult.welded}</p>
          </div>
          ...
          {linkFeeLoading ? (
            ...
            <div className="flex items-center justify-between p-2.5 sm:p-4 rounded-xl bg-white/[0.03] border border-white/[0.06]">
              ...
              <p className="font-black text-white text-[10px] sm:text-sm">
                ...
                <span className="text-blue-500/ text-[8px] sm:text-xs">({linkFeeInfo.feeToken})</span>
              </p>
            </div>
          ) : null}
          ...
          <div className="fixed inset-0 z-[80] flex items-center justify-center px-3 sm:px-4">
            ...
            >
              <div className="text-center space-y-2 sm:space-y-3">
                ...
                <p className="text-blue-500/ font-black text-[10px] sm:text-base">{unlinkTarget.name}</p>
                ...
                <div className="flex gap-2 sm:gap-3 pt-1.5 sm:pt-2">
                  ...
                  <button
                    onClick={() => {
                      ...
                      fetch(
                        `${SALVA_API_URL}/api/alias/estimate-unlink-fee?safeAddress=${baseUser?.safeAddress || user.safeAddress}&weldedName=${encodeURIComponent(unlinkTarget.name)}&registryAddress=${unlinkTarget.registryAddress}`
                      )
                        .then((r) => r.json())
          ...
          <div className="fixed inset-0 z-[80] flex items-center justify-center px-3 sm:px-4">
            ...
            >
              ...
              <div className="px-2.5 py-2 sm:px-3 sm:py-2.5 rounded-xl bg-white/[0.03] border border-white/[0.06] flex items-center justify-between text-[9px] sm:text-xs">
                ...
                {unlinkFeeLoading ? (
                  <span className="w-2.5 h-2.5 sm:w-3 sm:h-3 border border-white/20 border-t-blue-400 rounded-full animate-spin inline-block" />
        ...
        <div className="mb-7">
          ...
          <div className="relative flex items-center mb-6">
            ...
            <div className="mx-3 flex items-center gap-1.5">
              <span className="w-1 h-1 rounded-full bg-blue-500/40 block" />
              <span className="w-1.5 h-1.5 rounded-full bg-blue-500/60 block" />
              <span className="w-1 h-1 rounded-full bg-blue-500/40 block" />
            </div>
            <div className="flex-1 h-px bg-gradient-to-r from-transparent via-blue-500/30 to-transparent" />
          </div>
          ...
          <div className="relative flex items-center mt-6">
            ...
            <div className="mx-3 flex items-center gap-1.5">
              <span className="w-1 h-1 rounded-full bg-blue-500/30 block" />
              <span className="w-1.5 h-1.5 rounded-full bg-blue-500/40 block" />
              <span className="w-1 h-1 rounded-full bg-blue-500/30 block" />
            </div>
            <div className="flex-1 h-px bg-gradient-to-r from-transparent via-blue-500/20 to-transparent" />
          </div>
